...
//! entries stored in a [`Punnu`](crate::punnu::Punnu).
//!
//! A `Cacheable` type names its identity column (`Self::Id`), declares
//! a generated companion struct of [`Field`] accessors (`Self::Fields`),
//! and exposes a deterministic `id()` extractor. The companion struct
//! is normally produced by `#[derive(Cacheable)]` (see `sassi-macros`),
//! but hand impls are supported when the macro doesn't fit.
//!
...
/// Identity contract for entries stored in a [`Punnu`](crate::punnu::Punnu).
///
...
pub struct Field<T, V> {
    /// Column / serde-key name. Used by SQL emitters (djogi's `Q<T>`
    /// walker) and by debug formatting.
    pub name: &'static str,

    /// Memory-side extractor — pure function from `&T` to `&V`. Must be
    /// total (no panics) and deterministic.
    pub extract: fn(&T) -> &V,
...
// the generated `T::fields()` constructor.
impl<T: 'static, V: 'static> Default for Field<T, V> {
    fn default() -> Self {
        ...
        // generated `T::fields()` constructor instead.
        fn unwired<T, V>(_: &T) -> &V {
            panic!(
                "sassi: extractor invoked on a `Field` produced by `Default`; \
                 use the generated `T::fields()` constructor instead"
            )
        }
